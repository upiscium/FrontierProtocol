...
- Tier 1 has a final directional open-frame model with four horizontal facings, three static status variants, 18 referenced custom block textures, a central 3D core visible through four apertures, a local-Z rotating front gear, four-face state lights, and a rear bearing housing. Its dedicated codec preserves the subclass and facing contract, and Tier 1 alone disables occlusion while retaining full-cube collision. Kinetic input connects only to the rear face. If no display snapshot exists, the client uses BlockState status and configured minimum RPM so Ponder and first-packet rendering retain the intended visual state.
...
R11 automated verification covers all 12 Tier 1 facing/status blockstate variants, four static block-model resources, removal of three unused dynamic JSON models, 18 referenced custom 32x32 RGBA textures, absence of eight removed unused textures, item-model selection, snapshot and BlockState visual resolution, local-Z and wrap-safe animation calculations, orientation and occlusion invariants, dedicated codec reconstruction, display schema v2, and production-JAR packaging. Unit tests, datagen cleanliness, clean build, and all 39 GameTests pass. The dedicated server reached `Done` after the renderer addition without a client-class error. Focused in-game rendering passed for four facings, rear-only attachment, signed 32/64/128 RPM, three Grace cadence bands, Fast/Fancy, English/Japanese, GUI Scale 2/Auto, all required item contexts, JEI, and Operation/Coverage/Production Ponder.

Final item art: Compound complete; Cell complete. Final block art: Tier 1 complete with focused in-game visual verification; Tier 2 pending; Tier 3 pending. This partial asset completion does not make the alpha approved for public distribution.
...
- [x] Complete the focused R11 Tier 1 directional model, animation, state-light, inventory, JEI, and Ponder client smoke.
