...
- Tier 1 has a final directional open-frame model with four horizontal facings, three static status variants, a central 3D core visible through four apertures, a local-Z rotating front gear, four-face state lights, and a rear bearing housing. Its dedicated codec preserves the subclass and facing contract, and Tier 1 alone disables occlusion while retaining full-cube collision. Kinetic input connects only to the rear face.
...
R11 automated verification covers all 12 Tier 1 facing/status blockstate variants, four static block-model resources, removal of three unused dynamic JSON models, 26 custom 32x32 RGBA textures, item-model selection, local-Z and wrap-safe animation calculations, orientation and occlusion invariants, dedicated codec reconstruction, display schema v2, and production-JAR packaging. Unit tests, datagen cleanliness, clean build, and all 39 GameTests pass. The dedicated server also reached `Done` after the renderer addition without a client-class error and stopped normally. Focused in-game rendering verification remains pending and is not recorded as passed.
