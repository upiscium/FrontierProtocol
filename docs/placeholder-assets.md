...
R8 retained every placeholder block and item model. R10 replaces only the two non-block item icons. R11 replaces the Tier 1 placeholder with seven block-model resources, 26 custom 32x32 RGBA textures, 12 facing/status variants, and client-only animated parts. Goggle text, line-based range rendering, and Ponder storyboards add no particle texture.
