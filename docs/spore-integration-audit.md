...
A global block-write hook was rejected because those calls include unrelated entity behavior, item effects, explicit player actions, world generation, and maintenance of existing infection. R4-B is target-based: the source passed to `SpreadInfection` never suppresses an entire operation. Every visited target is checked, and helper methods that write to a different position check the actual `BlockPos` immediately before mutation.
...
| `FoliageSpread` | `SpreadFoliageAndConvert(Level,BlockState,BlockPos): (Lnet/minecraft/world/level/Level;Lnet/minecraft/world/level/block/state/BlockState;Lnet/minecraft/core/BlockPos;)V` | `SpreadInfection` loop | Base target plus helper offsets | Mixed | Server | HEAD target check | Rejects conversion of a protected visited target without stopping other positions. |
...
| `FoliageSpread` | `placeCropsFoliage(Level,BlockPos,BlockState): (Lnet/minecraft/world/level/Level;Lnet/minecraft/core/BlockPos;Lnet/minecraft/world/level/block/state/BlockState;)V` | `SpreadFoliageAndConvert` | `base` | Yes | Server | Covered by HEAD | No separate redirect is needed. |
...
| `FoliageSpread` | `convertFromJson(Level,BlockState,BlockPos): (Lnet/minecraft/world/level/Level;Lnet/minecraft/world/level/block/state/BlockState;Lnet/minecraft/core/BlockPos;)V` | `SpreadFoliageAndConvert` | `base` | Yes | Server | Covered by HEAD | Conversion writes to the visited target. |
...
| `FoliageSpread` | `convertBlocks(BlockState,Level,BlockPos): (Lnet/minecraft/world/level/block/state/BlockState;Lnet/minecraft/world/level/Level;Lnet/minecraft/core/BlockPos;)V` | `SpreadFoliageAndConvert` | `base` | Yes | Server | Covered by HEAD | Configured block conversion writes to the visited target. |
| `FoliageSpread` | `convertWood(Level,BlockState,BlockPos): (Lnet/minecraft/world/level/Level;Lnet/minecraft/world/level/block/state/BlockState;Lnet/minecraft/core/BlockPos;)V` | `SpreadFoliageAndConvert` | `base`; branch helper uses neighbors | Mixed | Server | HEAD plus branch redirects | Base writes and `FallingBlockEntity.fall` use the checked base; neighbor branches use actual-target guards. |
| `HiveTumor` | `SpreadFoliageAndConvert(Level,BlockState,BlockPos): (Lnet/minecraft/world/level/Level;Lnet/minecraft/world/level/block/state/BlockState;Lnet/minecraft/core/BlockPos;)V` | Entity tick spread | `base`, including post-default `CDUBlock.replaceCDU` | Yes | Server entity tick | Override HEAD target check | Prevents fall-through into the extra CDU mutation. |
| `Proto` | `SpreadFoliageAndConvert(Level,BlockState,BlockPos): (Lnet/minecraft/world/level/Level;Lnet/minecraft/world/level/block/state/BlockState;Lnet/minecraft/core/BlockPos;)V` | Entity tick and death spread | `base`, including post-default `CDUBlock.replaceCDU` | Yes | Server entity tick | Override HEAD target check | Prevents fall-through into the extra CDU mutation. |
...
Redirects name both the complete enclosing method descriptor and the complete `Level#setBlock(BlockPos,BlockState,int)` descriptor. Their `require` values are the audited call-site counts, so an incompatible Spore update fails during Mixin application instead of silently losing protection.
...
GameTests use actual Spore classes and deterministic mutation paths where available. `HiveTumor` and `Proto` use their post-default CDU replacement, while `Mound#placeStructureBlock` is reached through a test-only Mixin invoker without widening Spore's production API. Branch boundary cases cover north, south, west, and east chunk edges, exercise the real redirected helper, and separately assert the deterministic internal mutation guard. The former 4096-attempt crop-conversion test was removed; test success no longer depends only on eventually winning that random conversion branch.
