...
GameTests verify the loaded Create recipe serializers, exact ingredients and pattern, output counts, no-heat and fluid contract, absence of alternate recipes, item-capability Cell insertion, Compound rejection, and one-at-a-time consumption. The `r9_unheated_mixing` GameTest removes the Blaze Burner from Create's physical fixture, waits for the Basin heat level to become `NONE`, fills the Basin with exactly 100 mB Molten Plastic and all item inputs, and verifies that exactly one Compound reaches the output chest through Create logistics. Physical Deployer execution, Mechanical Crafter execution, and continuous funnel/chute Cell supply remain manual development-world checks until their dedicated acceptance tests run.
